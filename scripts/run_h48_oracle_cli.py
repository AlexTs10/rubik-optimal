#!/usr/bin/env python
"""Generate evidence for the public H48 oracle CLI batch command."""
...
def _write_table(root: Path, payload: dict[str, object], suffix: str) -> Path:
    rows = payload["rows"]
    filename = f"h48_oracle_cli{suffix}.tex" if suffix else "h48_oracle_cli.tex"
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"oracle CLI did not return JSON: {exc}; stderr={completed.stderr!r}") from exc

    rows = []
    for case, row in zip(cases, cli_payload.get("rows", [])):
        merged = dict(row)
    ...
    payload = {
        ...
        "cli_batch_wall_seconds": round(float(cli_payload.get("batch_wall_seconds", 0.0) or 0.0), 6),
        "input_count": len(cases),
        "all_exact": cli_payload.get("all_exact") is True,
        "all_verified": cli_payload.get("all_verified") is True,
        "rows": rows,
    }
    payload["passed"] = (
        ...
        and all(row.get("status") == "exact" and row.get("verified") is True for row in rows)
    )

    output = root / "results" / "processed" / f"h48_oracle_cli_seed_{args.seed}_{args.profile}{suffix}.json"
