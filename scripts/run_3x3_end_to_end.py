...
def _write_table(root: Path, rows: list[dict[str, object]]) -> Path:
    ...
    for row in rows:
        table_rows.append(
            f"{_tex(row['case_id'])} & \\code{{{row['input_kind']}}} & "
            f"\\code{{{row['distance_kind']}}} & {_tex(_short_solver(row['auto_solver']))} & "
            f"\\code{{{row['auto_status']}}} & "
    ...
    table_path.write_text(
        ...
        "\\hline\n"
        "Περίπτωση & Είσοδος & Απόσταση & Αυτόματη διαδρομή & Κατάσταση & Μήκος \\\\\n"
        "\\hline\n"
        + "\n".join(table_rows)
        + "\n\\hline\n\\end{tabular}\n\n"
        "{\\footnotesize \\code{exact_distance}: αποδεδειγμένη ακριβής απόσταση· \\code{lower_bound}: μόνο κάτω φράγμα· \\code{non_exact}: επαληθευμένη λύση χωρίς απόδειξη βελτιστότητας.}\n}\n",
        encoding="utf-8",
    )
    return table_path
