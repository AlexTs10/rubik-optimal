...
def _write_metadata_table(
    ...
    for metadata in metadatas:
        rows.append(
            ...
            f"{'Ναι' if metadata['complete'] else 'Όχι'} & "
            f"{metadata['max_distance']} & "
            f"{metadata['size_bytes']} & "
            f"{float(metadata['runtime_seconds']):.3f} \\\\"
        )
    header = (
        "Διαμέριση κόστους & Καταστάσεις & Πλήρης & Μέγ. απόσταση & Bytes & Χρόνος (s) \\\\\n"
        if additive
        else "Υποσύνολο & Καταστάσεις & Πλήρης & Μέγ. απόσταση & Bytes & Χρόνος (s) \\\\\n"
    )
    content = (
        "{\\small\n"
