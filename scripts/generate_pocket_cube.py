...
def main() -> int:
    ...
    rep_rows = [
        f"{_tex(row['case_id'])} & {row['solution_length']} & {_tex(row['solution'])} & \\code{{{row['status']}}} \\\\"
        for row in representatives
    ]
    representatives_tex = (
        "\\begin{tabular}{p{0.2\\textwidth}rp{0.34\\textwidth}p{0.16\\textwidth}}\n"
        "\\hline\n"
        "Περίπτωση & Μήκος & Λύση & Κατάσταση \\\\\n"
        "\\hline\n"
        + "\n".join(rep_rows)
        + "\n\\hline\n\\end{tabular}\n\n"
        "{\\footnotesize \\code{exact}: αποδεδειγμένα βέλτιστη.}\n"
    )
