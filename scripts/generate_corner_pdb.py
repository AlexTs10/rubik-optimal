...
def _write_metadata_table(root: Path, metadata: dict[str, object]) -> None:
    ...
    table_path.write_text(
        ...
        "Μέγεθος & Τιμή \\\\\n"
        "\\hline\n"
        f"Καταστάσεις προβολής & {metadata['state_count']} \\\\\n"
        f"Καταστάσεις που επισκεφθήκαμε & {metadata['visited_states']} \\\\\n"
        f"Πλήρης & {'Ναι' if metadata['complete'] else 'Όχι'} \\\\\n"
        f"Μέγιστη απόσταση & {metadata['max_distance']} \\\\\n"
        f"Μέγεθος δυαδικού (bytes) & {metadata['size_bytes']} \\\\\n"
        f"Χρόνος (s) & {float(metadata['runtime_seconds']):.3f} \\\\\n"
        f"Κάδοι κατανομής & {len(distribution)} \\\\\n"
        "\\hline\n"
        "\\end{tabular}\n"
        "}\n",
        encoding="utf-8",
