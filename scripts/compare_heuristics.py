...
def _write_table(path: Path, payload: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "\\hline\n",
        "Σύνολο & Περιπτώσεις & Ακριβείς & Mis. & Co. & Cor. & Ed. & Comb. & CPDB & Adm. \\\\\n",
        "\\hline\n",
    ]

    def _admissible(value: object) -> str:
        if value is None:
            return "--"
    ...
    for row in payload["by_corpus"]:
        lines.append(
            ...
            f"{_admissible(row['exact_rows_admissible'])} \\\\\n"
        )
    lines.extend(
        [
            "\\hline\n",
            "\\end{tabular}\n\n",
            "{\\footnotesize Οι στήλες δίνουν το μέσο κάτω φράγμα κάθε ευρετικής. "
            "\\textbf{Mis.}: λάθος τοποθετημένα cubies· \\textbf{Co.}: κλάδεμα συντεταγμένων· "
            "\\textbf{Cor.}: PDB γωνιών· \\textbf{Ed.}: PDB ακμών· \\textbf{Comb.}: συνδυασμένο φράγμα· "
            "\\textbf{CPDB}: αθροιστικό CPDB ακμών· \\textbf{Adm.}: το συνδυασμένο φράγμα ελέγχθηκε παραδεκτό.}\n",
            "}\n",
        ]
    )
