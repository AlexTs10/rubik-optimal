...
/*
Compute an admissible lower bound for the given cube and solver data.

For now this is implemented for H48 solvers only. The returned value is safe to
use as the minimum depth for an optimal search with the same table.

Return values:
   NISSY_ERROR_INVALID_CUBE   - The given cube is invalid.
   NISSY_ERROR_INVALID_SOLVER - The lower-bound probe is unavailable.
   NISSY_ERROR_NULL_POINTER   - One of the required arguments is null.
   NISSY_ERROR_DATA           - The data buffer is invalid.
   Any value >= 0             - The admissible lower bound.
*/
long long
nissy_lowerbound(
	const char cube[], /* NISSY_SIZE_CUBE */
