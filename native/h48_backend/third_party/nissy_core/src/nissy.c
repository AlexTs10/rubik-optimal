...
)
{
	...
	uint64_t offset;
	uint8_t normal, inverted;

	if (cube == NULL || solver == NULL || data == NULL) {
		LOG("[lowerbound] Error: required argument is NULL\n");
		return NISSY_ERROR_NULL_POINTER;
	}
	if (strncmp(solver, "h48", 3) != 0 && strcmp(solver, "optimal") != 0) {
		LOG("[lowerbound] Error: unsupported solver '%s'\n", solver);
		return NISSY_ERROR_INVALID_SOLVER;
	}
	if ((size_t)data % 8 != 0) {
		LOG("[lowerbound] Error: data buffer is not 8-byte aligned\n");
		return NISSY_ERROR_DATA;
	}

	oc = readcube(cube);
	if (!isconsistent(oc)) {
		LOG("[lowerbound] Error: cube is invalid\n");
		return NISSY_ERROR_INVALID_CUBE;
	}
	if (readtableinfo_n(data_size, data, 2, &info) != NISSY_OK) {
		LOG("[lowerbound] Error reading H48 data table\n");
		return NISSY_ERROR_DATA;
	}
	if (readtableinfo_n(data_size, data, 3, &fbinfo) != NISSY_OK ||
	    fbinfo.bits != 4 || fbinfo.type != TABLETYPE_SPECIAL) {
		LOG("[lowerbound] Error reading H48 fallback table\n");
		return NISSY_ERROR_DATA;
	}
	...
	normal = h48_prune_lookup_nocoord(oc.cube, &arg);
	inverted = h48_prune_lookup_nocoord(inverse(oc.cube), &arg);
	return MAX(normal, inverted);
}
