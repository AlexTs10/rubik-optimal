...
Το προαιρετικό όρισμα \code{STATE} δέχεται συμβολοσειρά ψηφίδων, ακολουθία κινήσεων από τη λυμένη κατάσταση ή τη λέξη \code{solved} (προεπιλογή). Η είσοδος ελέγχεται για φυσική εγκυρότητα πριν κληθεί οποιοσδήποτε επιλυτής. Η επιλογή \code{--solver} δέχεται τις ακόλουθες τιμές:
...
\item Επιλυτές χαρτοφυλακίου: \code{race-optimal}, \code{resident-race-optimal} και \code{universal-optimal}, οι οποίοι συνδυάζουν παράλληλα ή κλιμακωτά τα ακριβή backends και τα πιστοποιητικά (Ενότητα~\ref{sec:cli-portfolio}).
\end{itemize}

Η επιλογή \code{--max-depth} (προεπιλογή 10) ερμηνεύεται ανά επιλυτή: μόνο ο επιλυτής \code{korf} την τηρεί κατά γράμμα, οι ακριβείς επιλυτές σε C/C++ την αυξάνουν σιωπηρά σε τουλάχιστον 20, οι επιλυτές δύο φάσεων παράγουν από αυτήν φασικά όρια τουλάχιστον 12 και 18, και ο επιλυτής Thistlethwaite αυξάνει τα όρια των σταδίων 3 και 4 σε τουλάχιστον 13 και 15. Τιμές κάτω από τα ελάχιστα αυτά αυξάνονται αυτόματα και δεν απορρίπτονται, ώστε κανένας επιλυτής να μη γίνεται μη πλήρης από λανθασμένη διαμόρφωση. Ο Πίνακας~\ref{tab:cli-solve} συνοψίζει τις υπόλοιπες γενικές επιλογές της υποεντολής.
...
\code{--threads} & αυτόματη & Νήματα των αναζητήσεων σε C/C++· η προεπιλογή προκύπτει από το φορτίο του μηχανήματος ή τις μεταβλητές περιβάλλοντος \code{RUBIK_OPTIMAL_THREADS} / \code{RUBIK_OPTIMAL_H48_THREADS}. \\
\code{--split-depth} & 3 & Βάθος διαμοιρασμού εργασιών στον παράλληλο IDA* σε C/C++. \\
\code{--native-child-order} & \code{heuristic-desc} & Διάταξη διαδόχων στον IDA* σε C/C++ (\code{heuristic-asc}, \code{move}). \\
...
Οι επιλογές του Πίνακα~\ref{tab:cli-h48} είναι κοινές στις υποεντολές \code{solve}, \code{distance} και \code{oracle} και ρυθμίζουν τη μηχανή H48. Η σημαία \code{--h48-trusted-table} είναι ασφαλής μόνο για πίνακες που έχουν παραχθεί τοπικά και ταυτοποιηθεί από τα αποθηκευμένα μεταδεδομένα και αθροίσματα ελέγχου. Η σημαία \code{--h48-preload-table} προθερμαίνει σειριακά τις σελίδες του πίνακα πριν από την αναζήτηση: προορίζεται για την πιστοποίηση δύσκολων καταστάσεων μετά από ψυχρή εκκίνηση και δεν συνεπάγεται γενική επιτάχυνση κάθε κλήσης· οι μετρημένες επιπτώσεις των δύο σημαιών παρουσιάζονται στο Κεφάλαιο~\ref{chap:experiments}.
...
\code{--threads} & αυτόματη & Νήματα των αναζητήσεων σε C/C++. \\
\code{--optimal-native} & ανενεργή & IDA* σε C/C++ με τους πίνακες προτύπων μετά την BFS. \\
\code{--h48-native} & ανενεργή & Η μηχανή H48 σε C/C++ μετά την BFS, με είσοδο απευθείας την κατάσταση. \\
\hline
\end{tabular}
...
\code{--h48-upper-bound-proof-timeout} & 0 & Δευτερόλεπτα απόδειξης H48, με χρονικό όριο, ότι δεν υπάρχει λύση κάτω από επαληθευμένο άνω φράγμα. \\
\code{--h48-upper-bound-proof-max-gap} & 1 & Μέγιστο χάσμα άνω και κάτω φράγματος για να εκτελεστεί η απόδειξη. \\
\code{--native-korf-}\allowbreak\code{upper-bound-proof-timeout} & 0 & Δευτερόλεπτα απόδειξης Korf/IDA* σε C/C++ ενός φράγματος κάτω από επαληθευμένη λύση. \\
\code{--native-korf-}\allowbreak\code{upper-bound-proof-max-gap} & 1 & Αντίστοιχο μέγιστο χάσμα για την απόδειξη σε C/C++. \\
\end{longtable}
}
