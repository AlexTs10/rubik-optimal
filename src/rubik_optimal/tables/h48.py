...
def write_h48_latex_table(root: Path, rows: list[dict[str, object]]) -> Path:
    ...
    def display_status(value: object) -> str:
        status = str(value)
        if status == "adopted_existing_table_metadata":
            return "εξωτερικά μεταδεδομένα"
        if status == "generated":
            return "παραγωγή τοπικά"
        return status
    ...
    for row in rows:
        body.append(
            ...
            f"{tex(row['checksum_sha256'][:12])} & {tex(display_status(row['generation_status']))} \\\\"
        )
    table_path.write_text(
        "{\\small\n"
        "\\begin{tabular}{llrll}\n"
        "\\hline\n"
        "Επιλυτής & Προφίλ & Bytes & Πρόθεμα SHA-256 & Κατάσταση \\\\\n"
        "\\hline\n"
        + "\n".join(body)
        + "\n\\hline\n\\end{tabular}\n"
        "}\n",
