...
def _write_latex_metadata_table(root: Path, rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        body.append(
            f"{_tex(_display_table_name(row['table_name']))} & \\code{{{row['table_kind']}}} & "
            f"{row['domain_size']} & {row['entry_count']} & {len(row['moves'])} \\\\"
        )
    table_path.write_text(
        "{\\small\n\\begin{tabular}{L{0.27\\textwidth}L{0.18\\textwidth}rrr}\n"
        "\\hline\n"
        "Πίνακας & Είδος & Πεδίο & Καταχωρήσεις & Κινήσεις \\\\\n"
        "\\hline\n"
        + "\n".join(body)
        + "\n\\hline\n\\end{tabular}\n\n"
        "{\\footnotesize \\code{move_table}: πίνακας μετάβασης κινήσεων· \\code{pruning_table}: πίνακας κλαδέματος (κάτω φράγμα).}\n}\n",
        encoding="utf-8",
    )
