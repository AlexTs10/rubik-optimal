...
def _write_thesis_tables(root: Path, rows: list[dict[str, object]], summary: dict[str, object]) -> None:
    ...
    thesis_hidden_solvers = {"kociemba_two_phase_adapter"}
    thesis_rows = [row for row in rows if str(row["solver"]) not in thesis_hidden_solvers]
    ...
    summary_solver_names = [
        name
        for name in summary["solvers"].keys()  # type: ignore[union-attr]
        if name not in thesis_hidden_solvers
    ]
    ...
    for solver in solver_names:
        data = summary["solvers"][solver]  # type: ignore[index,union-attr]
    ...
    (tables / "benchmark_summary.tex").write_text(
        ...
        "\\hline\n"
        "Επιλυτής & Περιπτώσεις & Επαληθευμένες & Διάμεσος χρόνος (s) & Καταστάσεις \\\\\n"
        "\\hline\n"
        + "\n".join(table_rows)
        + "\n\\hline\n\\end{tabular}\n\n"
        "{\\footnotesize \\code{exact}: αποδεδειγμένα βέλτιστη· \\code{non_exact}: επαληθευμένη χωρίς απόδειξη βελτιστότητας· \\code{lower_bound}: μόνο κάτω φράγμα.}\n}\n",
        encoding="utf-8",
    )
    (tables / "algorithm_status.tex").write_text(
        "{\\small\n\\begin{tabular}{L{0.17\\textwidth}L{0.29\\textwidth}L{0.25\\textwidth}L{0.17\\textwidth}}\n"
        "\\hline\n"
        "Αλγόριθμος & Κατάσταση υλοποίησης & Κατάσταση βελτιστότητας & Τεκμήρια \\\\\n"
        "\\hline\n"
        "BFS & Εξαντλητική αναζήτηση σε μικρό βάθος & \\code{exact} εντός του βάθους & tests/results \\\\\n"
        "Korf/IDA* & Περιορισμένο IDA* σε Python με πίνακες προβολής και κάτω φράγματα από τα PDB γωνιών/ακμών & \\code{exact} όταν ολοκληρώνεται & tests + PDB \\\\\n"
        "Korf native optimal & IDA* σε C++ για ολόκληρο τον κύβο, με πλήρη PDB γωνιών και 6 ακμών & \\code{exact} όταν ολοκληρώνεται & optimal evidence \\\\\n"
        "Kociemba native & Περιορισμένο δύο φάσεων: ακριβής ευρετική φάσης 1 με συμμετρία και κλάδεμα προβολής στη φάση 2 & \\code{non_exact}, επαληθευμένη όταν ολοκληρώνεται & tests + tables \\\\\n"
        "Thistlethwaite native & Άπληστη κάθοδος τεσσάρων φάσεων πάνω σε ακριβείς πίνακες απόστασης BFS ανά φάση & \\code{non_exact}, επαληθευμένη, εγγυημένος τερματισμός & tests + tables \\\\\n"
        "\\hline\n\\end{tabular}\n\n"
        "{\\footnotesize \\code{exact}: αποδεδειγμένα βέλτιστη· \\code{non_exact}: επαληθευμένη χωρίς απόδειξη βελτιστότητας.}\n}\n",
        encoding="utf-8",
    )
    depth_rows = [
    ...
    (tables / "benchmark_dataset_summary.tex").write_text(
        "\\begin{tabular}{lrrr}\n"
        "\\hline\n"
        "Σύνολο & Περιπτώσεις & Ελάχ. βάθος & Μέγ. βάθος \\\\\n"
        "\\hline\n"
        + "\n".join(dataset_rows)
        + "\n\\hline\n\\end{tabular}\n",
        encoding="utf-8",
    ...
    (tables / "benchmark_case_catalog.tex").write_text(
        ...
        "\\hline\n"
        "Περίπτωση & Σύνολο & Βάθος & Ανακάτεμα \\\\\n"
        "\\hline\n"
        + "\n".join(catalog_rows)
        + "\n\\hline\n\\end{tabular}\n",
        encoding="utf-8",
    ...
    def kind_cell(kind: str) -> str:
        # \code uses \detokenize, so the token inside must keep raw underscores.
        return f"\\code{{{kind}}}"

    def method_human(method: str) -> str:
        import re as _re

        bfs = _re.fullmatch(r"bfs_depth_(\d+)", method)
        if bfs:
            return f"BFS, βάθος {bfs.group(1)}"
        ida = _re.fullmatch(r"ida_star_depth_(\d+)", method)
        if ida:
            return f"IDA*, βάθος {ida.group(1)}"
        if method == "combined_table_lower_bound":
            return "συνδυασμένο κάτω φράγμα"
        return tex(method)
    ...
    distance_rows = [
        f"{kind_cell(kind)} & {method_human(method)} & {count} \\\\"
    ...
    (tables / "distance_recognition_summary.tex").write_text(
        ...
        "\\hline\n"
        "Είδος απόστασης & Μέθοδος & Περιπτώσεις \\\\\n"
        "\\hline\n"
        + "\n".join(distance_rows)
        + "\n\\hline\n\\end{tabular}\n\n"
        "{\\footnotesize \\code{exact_distance}: αποδεδειγμένη ακριβής απόσταση· \\code{lower_bound}: μόνο κάτω φράγμα.}\n",
        encoding="utf-8",
    )
    ...
    status_rows = []
    for solver in solver_names:
        counts = Counter(str(row["status"]) for row in thesis_rows if row["solver"] == solver)
        status_rows.append(
    ...
    (tables / "solver_status_counts.tex").write_text(
        ...
        "\\hline\n"
        "Επιλυτής & \\code{exact} & \\code{non_exact} & \\code{lower_bound} & \\code{timeout} & Άνευ & Αποτυχία \\\\\n"
        "\\hline\n"
        + "\n".join(status_rows)
        + "\n\\hline\n\\end{tabular}\n\n"
        "{\\footnotesize \\code{exact}: αποδεδειγμένα βέλτιστη· \\code{non_exact}: επαληθευμένη χωρίς απόδειξη βελτιστότητας· \\code{lower_bound}: μόνο κάτω φράγμα· \\code{timeout}: λήξη χρόνου.}\n",
        encoding="utf-8",
    )

    runtime_rows = []
    for solver in solver_names:
        runtimes = [float(row["runtime_seconds"]) for row in thesis_rows if row["solver"] == solver]
        runtime_rows.append(
    ...
    (tables / "solver_runtime_summary.tex").write_text(
        ...
        "\\hline\n"
        "Επιλυτής & Ελάχ. (s) & Διάμεσος (s) & Μέγ. (s) \\\\\n"
        "\\hline\n"
        + "\n".join(runtime_rows)
        + "\n\\hline\n\\end{tabular}\n",
        encoding="utf-8",
    ...
    (tables / "solver_solution_length_summary.tex").write_text(
        ...
        "\\hline\n"
        "Επιλυτής & Επαληθευμένες & Ελάχ. & Διάμεσος & Μέγ. \\\\\n"
        "\\hline\n"
        + "\n".join(length_rows)
        + "\n\\hline\n\\end{tabular}\n",
        encoding="utf-8",
    ...
    node_rows = []
    for solver in solver_names:
        solver_rows = [row for row in thesis_rows if row["solver"] == solver]
    ...
    (tables / "solver_node_summary.tex").write_text(
        ...
        "\\hline\n"
        "Επιλυτής & Διάμεσος κόμβων επέκτασης & Διάμεσος κόμβων παραγωγής \\\\\n"
        "\\hline\n"
        + "\n".join(node_rows)
        + "\n\\hline\n\\end{tabular}\n",
        encoding="utf-8",
    ...
    status_by_case = {(str(row["case_id"]), str(row["solver"])): row for row in thesis_rows}
    matrix_rows = []
    ...
    (tables / "benchmark_case_status_matrix.tex").write_text(
        "\\begin{tabular}{p{0.25\\textwidth}rlll}\n"
        "\\hline\n"
        "Περίπτωση & Βάθος & Korf & Koc. native & Thistle. \\\\\n"
        "\\hline\n"
        + "\n".join(matrix_rows)
        + "\n\\hline\n\\end{tabular}\n\n"
        "{\\footnotesize Κάθε κελί: κατάσταση/μήκος λύσης. \\textbf{E}: \\code{exact} (αποδεδειγμένα βέλτιστη)· \\textbf{N}: \\code{non_exact} (επαληθευμένη χωρίς απόδειξη βελτιστότητας)· \\textbf{LB}: \\code{lower_bound} (μόνο κάτω φράγμα).}\n",
        encoding="utf-8",
    )
