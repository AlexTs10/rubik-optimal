...
- `thesis/main.pdf`: thesis-first academic text and compact appendices.
- `REPRODUCIBILITY.md`: canonical reproduction guide and full verification runbook.
- `docs/cli_reference.md`: detailed operational CLI reference moved out of the thesis PDF.
- `docs/source_state_reproducibility.md`: final-source provenance policy, including the retained H48 table boundary.
...
Every number printed in the thesis traces to stored evidence under the result artifacts; the verification gates below re-check that property mechanically. The thesis builds with `latexmk` (see PDF convention below).
...
- Optional external third-party Kociemba adapter (PyPI `kociemba`, GPLv2) as a non-exact comparison baseline and fast upper-bound seed for the oracle certificate path, separate from the native algorithms above.
